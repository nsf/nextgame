...
static inline float Gradient(const Vec3 &orig, const Vec3 &grad, const Vec3 &p)
{
...
static inline Vec3 RandomGradient3D(Rnd &g)
{
...
template <typename Rnd>
static inline Vec2 RandomGradient2D(Rnd &g)
{
	std::uniform_real_distribution<float> _2pi(0.0f, MATH_PI * 2.0f);
	const float angle = _2pi(g);
...
Vec3 Noise3D::get_gradient(int x, int y, int z) const
{
	int idx =
...
void Noise3D::get_gradients(Vec3 *origins, Vec3 *grads,
...
Vec2 Noise2D::get_gradient(int x, int y) const
{
	int idx =
		m_permutations[x & 255] +
		m_permutations[y & 255];
...
void Noise2D::get_gradients(Vec2 *origins, Vec2 *grads, float x, float y) const
{
	float x0f = floorf(x);
	float y0f = floorf(y);
	int x0 = x0f;
