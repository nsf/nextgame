...
Quat::Quat(const Vec3 &dir, float angle)
{
...
Quat::Quat(const Vec3 &u, const Vec3 &v)
{
	Vec3 w = cross(u, v);
	...
	if (len2 < MATH_EPSILON && real < 0) {
		w = std::abs(u.x) > std::abs(u.z) ?
			Vec3(-u.y, u.x, 0) / Vec3(length(u.XY())) :
			Vec3(0, -u.z, u.y) / Vec3(length(u.YZ()));
		this->x = w.x;
		this->y = w.y;
		this->z = w.z;
		this->w = 0;
...
Vec3 Quat::rotate(const Vec3 &v) const
{
	...
	*/
	const Vec3 xyz(x, y, z);
	return v + Vec3(2) * cross(xyz, cross(xyz, v) + Vec3(w) * v);
}
...
Quat Quat_LookAt(const Vec3 &v)
{
	const Vec3 unit = -Vec3_Z();
	const Vec3 around_y = normalize(Vec3(v.x, 0, v.z));
