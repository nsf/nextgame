...
Vec3 operator*(const Mat4 &l, const Vec3 &r)
{
	return {
...
Vec3 operator*(const Vec3 &l, const Mat4 &r)
{
	return {
...
Mat4 Mat4_Rotate(const Vec3 &axis, float angle)
{
	Mat4 m;
...
Vec4 Vec4_MiniOrtho2D(float left, float right, float bottom, float top, const Vec2 &offset)
{
	float x = 2.0f / (right - left);
	float y = 2.0f / (top - bottom);
...
Vec4 mini_ortho_translate(const Vec4 &miniortho, const Vec2 &offset)
{
	return {
		miniortho.x,
		miniortho.y,
...
Vec4 Vec4_MiniPerspective3D(float vfov, float aspect, float znear, float zfar)
{
